...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stok < 0) {
        ...
        $stmt = $pdo->prepare("UPDATE products SET stok = :stok WHERE id = :id");
        $stmt->execute([':stok' => $stok, ':id' => $id]);
        header('Location: index.php?stok=1');
        exit;
    }
}
?>
...
<head>
    ...
    <title>Update Stok - TokoKu</title>
    <meta name="description" content="Perbarui stok produk di sistem manajemen TokoKu.">
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        /* STOK PAGE SPECIFIC */
        .stok-display{text-align:center;padding:1.5rem 0 1rem;}
        .stok-current{font-size:3.5rem;font-weight:800;background:linear-gradient(135deg,#ec4899,#f43f5e);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1;}
        .stok-current-label{font-size:0.85rem;color:#7c6e7f;margin-top:6px;font-weight:500;}
        .quick-btns{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:0.5rem;}
        .quick-btn{padding:6px 14px;border-radius:8px;font-size:0.8rem;font-weight:600;cursor:pointer;border:1.5px solid #f9c0d8;background:#fdf2f8;color:#db2777;transition:all 0.2s;}
        .quick-btn:hover{background:#fce7f3;}
        .quick-label{font-size:0.8rem;color:#7c6e7f;margin-bottom:6px;font-weight:500;}
...
<div class="main-container">
    <div class="page-header">
        <div>
            <h1 class="page-title">📦 Update Stok</h1>
            <p class="page-subtitle">Perbarui stok untuk: <strong><?= htmlspecialchars($product['name']) ?></strong></p>
        </div>
    ...
    <div class="card form-card">
        <div style="padding: 2rem;">
            ...
            <div class="stok-display">
                <div class="stok-current"><?= (int)$product['stok'] ?></div>
                <div class="stok-current-label">Stok saat ini</div>
            </div>

            <form action="updatestok.php?id=<?= $id ?>" method="POST">
                <div class="form-group">
                    <label class="form-label" for="stok">Stok Baru</label>
                    <div class="quick-label">Pilih cepat:</div>
                    <div class="quick-btns">
                        <?php foreach ([0, 5, 10, 25, 50, 100] as $val): ?>
                        <button type="button" class="quick-btn"
                                onclick="document.getElementById('stok').value=<?= $val ?>">
                            <?= $val ?>
                        </button>
                        <?php endforeach; ?>
                    </div>
                    <input type="number" id="stok" name="stok" class="form-control"
                           value="<?= (int)$product['stok'] ?>" min="0" required>
                </div>
